...
class FilterApplier
{
     ...
     * @param Closure(string, string|null): string|bool $filterableResolver
     * @param Closure(string, string|null): string|bool $expandableResolver
     */
    public function __construct(
        private readonly Request $request,
        private readonly Closure $filterableResolver,
        private readonly Closure $expandableResolver,
    ) {
    }
    ...
    public function apply(Builder $builder): void
    {
        ...
        if (empty($filter)) {
            ...
            if (!empty($matches[1])) {
                $filter = "{$builder->getModel()->getKeyName()} eq '{$matches[1]}'";
            } else {
                return;
            }
        }

        $this->appendFilterQuery(strval($filter), $builder);
    }
    ...
    private function applySimpleExpressions(string $filter, Builder $builder): void
    {
        ...
        foreach ($groupedFilters as $filterPart) {
             ...
             * @var string $column
             * @var string $operator
             * @var string|array<int, string>|null $value
             * @var string $lambda
             * @var string $relation
             */
    ...
    private function splitInput(string $input, bool $inverseOperator = false): array
    {
        ...
        // Remove nulls in case of invalid tokens
        $tokens = array_filter($tokens, fn($token) => $token !== null);
        $tokens = array_values($tokens);

        // Determine format based on token count and presence of lambda expressions
        $lambda = '';
        $relation = '';
        if (count($tokens) >= 7 && ($tokens[1] == 'any' || $tokens[1] == 'all')) {
            // Lambda expression format: relation/lambda(alias: condition)
            // Example: relatedModels/any(s: s/name eq 'John')
            // Tokens: [relation, any, alias, :, alias/name, eq, 'John']

            // First token is the relation, second is the lambda function (any/all),
            // Third is the lambda variable. We need to find the column and condition.

            // If there's no dot notation, the structure is: relation, any/all, alias, column, operator, value
    ...
    private function isPropertyFilterable(string $property, ?string $className = null): string|bool
    {
        return ($this->filterableResolver)($property, $className);
    }

    private function isPropertyExpandable(string $property, ?string $className = null): string|bool
    {
        return ($this->expandableResolver)($property, $className);
    }
}
