...
use Aqqo\OData\Filters\FilterInterface;
 ...
 */
trait FilterTraitNew
{
    /**
     * Add filters to the query based on the OData $filter parameter
     *
     * @return void
    ...
    public function addFilters(): void
    {
        ...
        if (empty($filter)) {
            // Handle URL-based filtering (e.g., /users(123))
    ...
    private function applySimpleExpressions(string $filter, Builder $builder): void
    {
        ...
        foreach ($groupedFilters as $filterPart) {
            ...
            if ($lambda) {
                if ($expandable = $this->isPropertyExpandableForNewSystem($relation)) {
                    if ($lambda == 'any') {
    ...
    private function isValidFilter(string $column, string $operator, string|array|null $value, Builder $builder): string|bool
    {
        ...
        $filterable = $this->isPropertyFilterableForNewSystem($column, $shortName);
    ...
    protected function applyRelationshipCondition(Builder $builder, string $function, string $relation, string $condition): void
    {
        ...
        if (!$this->isPropertyFilterableForNewSystem("{$relation}.{$column}")) {
            return;
        }

        $expandable = $this->isPropertyExpandableForNewSystem($relation);
    ...
    private function splitInput(string $input, bool $inverseOperator = false): array
    {
        ...
        if (isset($tokens[1]) && strtolower($tokens[1]) === 'in') {
            $column = $tokens[0];
            $operator = \Aqqo\OData\Utils\OperatorUtils::mapOperator($tokens[1], $inverseOperator);
            ...
            $column = $tokens[2];
            $operator = \Aqqo\OData\Utils\OperatorUtils::mapOperator($tokens[0], $inverseOperator);
            $value = \Aqqo\OData\Utils\OperatorUtils::getValueBasedOnOperator($tokens[0], $tokens[4]);
            ...
            $operatorIndex = 6;
            if (isset($tokens[7]) && \Aqqo\OData\Utils\OperatorUtils::isValidOperator(strtolower($tokens[7]))) {
                $operatorIndex = 7;
                $column = "{$tokens[5]}.{$tokens[6]}";
            }

            $operator = \Aqqo\OData\Utils\OperatorUtils::mapOperator($tokens[$operatorIndex], ($tokens[1] == 'all'));
            ...
            if (strtolower($tokens[$operatorIndex]) === 'in') {
                ...
                if (!empty($values)) {
                    $value = \Aqqo\OData\Utils\OperatorUtils::getValueBasedOnOperator('in', $values);
                } else {
                    throw new \Exception('Invalid IN operator syntax');
                }
            } else {
                $value = isset($tokens[$operatorIndex + 1]) ? \Aqqo\OData\Utils\OperatorUtils::getValueBasedOnOperator($tokens[$operatorIndex], $tokens[$operatorIndex + 1]) : '';
            }
            ...
            $column = $tokens[0];
            $operator = \Aqqo\OData\Utils\OperatorUtils::mapOperator($tokens[1], $inverseOperator);
            $value = \Aqqo\OData\Utils\OperatorUtils::getValueBasedOnOperator($tokens[1], $tokens[2]);
        }
    ...
    private function splitFilter(string $expr): array
    {
        ...
        if (!\Illuminate\Support\Str::contains($expr, ['(', ')', ' and ', ' or '])) {
            // If commas are present, split by commas; otherwise, return the trimmed expression
            return \Illuminate\Support\Str::contains($expr, ',')
    ...
    /**
     * Check if a property is filterable (fallback method for new filter system)
     *
     ...
     * @throws \ReflectionException
     */
    protected function isPropertyFilterableForNewSystem(string $property, string $modelName): bool|string
    {
        // First try to use the existing AttributesTrait method if available
        if (method_exists($this, 'isPropertyFilterable')) {
            $result = $this->isPropertyFilterable($property, $modelName);
            if ($result !== false) {
                return $result;
            }
        }
        
        // Fallback implementation - check if the property exists in the model
    ...
    /**
     * Check if a property is expandable (relationship) - fallback for new filter system
     *
     * @param string $property
     * @return string|false
     */
    protected function isPropertyExpandableForNewSystem(string $property): string|false
    {
        // First try to use the existing AttributesTrait method if available
        if (method_exists($this, 'isPropertyExpandable')) {
            $result = $this->isPropertyExpandable($property);
            if ($result !== false) {
                return $result;
            }
        }
        ...
        // If the relationship doesn't exist, return false
        return false;
    }
}
